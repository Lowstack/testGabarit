...
ExportChina <- wb_data(country = "CHN", indicator ="NE.EXP.GNFS.CD")
ExportChina
...
ExportChina %>% 
  ...
  geom_point(colour="red")+
  geom_line(colour="blue")+ 
  labs(title = "Exportations chinoises en $USD de 1970 à 2020", x = "Années", y = "Exportations en $USD" )+
  scale_y_continuous(expand = c(0,0),
                     limits=c(0, 3000000000000), 
                     labels = label_number(suffix = "T", scale = 1e-11),
                     breaks=seq(from = 0, to = 3000000000000, by = 500000000000)
  ...
  theme_classic()+
  ggsave(
    filename = "bigGraphyBoiiChina.pdf",
    scale = 1,
    device = "pdf",
    width = 8,
    height = 8
